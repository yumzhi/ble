...
;  External Device Setup Registers (Accessed via Chip Select Pins)
   AREA   Sys_boot_chimera,CODE,READONLY
...
;total stack small than 1496
;current total stack : 1344 bytes
; IRQ_STACK   EQU    SRAM_LIMIT
; FIQ_STACK   EQU    IRQ_STACK-512 ;FIQ_STACK   EQU    IRQ_STACK-512
; SVC_STACK   EQU    FIQ_STACK-384 ;FIQ_STACK-512;FIQ_STACK-384 ;; SVC_STACK   EQU    FIQ_STACK-512
; USR_STACK   EQU    SVC_STACK-640 ;SVC_STACK-448;SVC_STACK-576;;USR_STACK   EQU    SVC_STACK-1024
; SYS_STACK   EQU    USR_STACK-256 ;USR_STACK-128
...
_SYSboot_Setup_Stacks_Disable_Intr
    ...
    MOV     R0, #MODE_SVC:OR:I_BIT:OR:F_BIT ;SVC Supervisor Calls access to system resources from an operating system, SWI instruction is executed.
    MSR     CPSR_c, R0                      ;clear status register I bit(IRQ),F bit(FIRQ)
    LDR     R13, =SVC_STACK
    LDR     R0, =0xDEADBEEF
    STR     R0, [R13]
    BX lr
