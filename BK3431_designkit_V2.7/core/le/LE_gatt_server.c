...
#define MAX_GATT_SERVER_NUM_ENTRIES         200//400
...
#define _FIND_ME_TI_SIM_DB_LEN 4 //NEW
...
GATT_DB_setup_function GATT_DB_setup_handlers[GATT_NO_OPERATION] = {
...
t_error GATTserver_Init(void)
{
    ...
    const GATT_DB_t GATT_DB_SELECT[DB_LEN]={GAP_GATT_DB,GATT_GATT_DB,DEVICE_INFO_GATT_DB,
                                            USER_DEF_FFE0_GATT_DB};//NEW
#endif
    u_int8 GATT_DB_SELECT_index = 0;
    Gatt_Set_MTU_Size(23);
...
//							 DEBUG_MSG(0x53);
//							 DEBUG_MSG(p_result[1]);
//							 DEBUG_MSG(p_result[0]);
//							 DEBUG_MSG(p_result[3]);
//							 DEBUG_MSG(p_result[2]);
        ...
        // Now write the Value...
        {
            ...
            //if (length <= value_len)
            {
                ...
                if ((!descriptor) & (p_current_attrib->attrib_value.value.local_notification))//for ATT_WRITE command
                {
                    ...
                    if(GATTserver_DB[i].attrib_UUID.UUID_val == 0xFFE1)
                    {
                        for (j=0;j<length;j++)
                        {
                            Beken_UART_TxChar(p_attrib_value[j]);    //frank 160623
                        }
                    }
