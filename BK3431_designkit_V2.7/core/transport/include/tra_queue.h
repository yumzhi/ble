 ...
 * DATE:           20 March 2000
 *
 * SOURCE CONTROL: $Id: tra_queue.h,v 1.1 2012/05/09 00:10:24 garyf Exp $
 *
 * LICENSE:
...
enum 
{
...
#if 0 // LE Included
    LE_LLC_IN_Q  = LMP_OUT_Q + MAX_ACTIVE_DEVICE_LINKS,
    LE_LLC_OUT_Q = LE_LLC_IN_Q + MAX_ACTIVE_DEVICE_LINKS,
#if (PRH_BS_CFG_SYS_TRACE_VIA_HCI_SUPPORTED==1)
    TCI_DEBUG_Q   = LE_LLC_OUT_Q + MAX_ACTIVE_DEVICE_LINKS,
    L2CAP_IN_Q    = TCI_DEBUG_Q + 1,
#else
    L2CAP_IN_Q    = LE_LLC_OUT_Q + MAX_ACTIVE_DEVICE_LINKS,
#endif
    ...
    L2CAP_IN_Q    = TCI_DEBUG_Q + 1,
#else
    L2CAP_IN_Q    = LMP_OUT_Q + MAX_ACTIVE_DEVICE_LINKS,/* 3 + 4 = 7 */
#endif
#endif
    L2CAP_OUT_Q   = L2CAP_IN_Q + MAX_ACTIVE_DEVICE_LINKS,/* 7 + 4 = 11 */
    /* 
...
#else
    TRA_QUEUE_TOTAL_NUM_QUEUES = L2CAP_OUT_Q + MAX_ACTIVE_DEVICE_LINKS + 1/* 11+4+1 */
#endif
...
typedef struct s_data_queue
{
    ...
    u_int16 next_free_pointer;          /* pointer to next free header       */
    t_q_descr **free_header_list;       /* Array of pointers to free headers */
    u_int8  *heap;                      /* Pointer to queue data store       */
} t_data_queue;
 ...
 ****************************************************************************/
t_q_descr *BTq_Enqueue(u_int8 q_type, t_deviceIndex device_index, t_length);
...
u_int16 BTq_Get_Total_Bytes_Enqueued(u_int8 q_type, t_deviceIndex device_index);

/*
 * Inlined functions 
 */
#ifdef __USE_INLINES__
#include "tra_queue_impl.h"
#endif
